...
function projectVillage(village: Village) {
  return {
    x: Math.round(((village.lon - BOUNDARY_GEO_BOUNDS.minLon) / (BOUNDARY_GEO_BOUNDS.maxLon - BOUNDARY_GEO_BOUNDS.minLon)) * (RASTER_W - 1)),
    y: Math.round(((BOUNDARY_GEO_BOUNDS.maxLat - village.lat) / (BOUNDARY_GEO_BOUNDS.maxLat - BOUNDARY_GEO_BOUNDS.minLat)) * (RASTER_H - 1)),
  };
}
