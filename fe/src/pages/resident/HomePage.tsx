...
export function ResidentHomePage() {
  ...
  return <div className="space-y-5">
    ...
    <div className="space-y-4">{inbox.data?.map((alert) => <article key={alert.alert_id} className="rounded-lg border border-border bg-surface-2 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><ShieldAlert className="text-accent" size={20} /><span className="font-semibold">{alert.hazard_type ?? "Cảnh báo"}</span></div><div className="flex gap-2"><HazardLevelBadge level={alert.level as 1 | 2 | 3 | 4 | 5} compact /><TierBadge tier={alert.tier as "prepare" | "go_now"} size="sm" /></div></div><h2 className="mt-4 text-lg font-semibold text-fg-strong">{alert.what_happened}</h2><p className="mt-2 text-sm text-muted">{alert.danger_description}</p><p className="mt-3 rounded-lg bg-surface px-3 py-2 text-sm font-medium text-fg">{alert.action_instruction}</p><p className="mt-3 text-xs text-muted">Thực hiện trước: {new Date(alert.deadline_at).toLocaleString("vi-VN")}</p>{alert.is_locale_fallback && <p className="mt-2 text-xs text-muted">Đang hiển thị nội dung tiếng Việt đã được phát hành.</p>}<div className="mt-4 flex flex-wrap gap-2"><Button variant="secondary" className="min-h-9 px-3" isLoading={acknowledge.isPending} onClick={() => acknowledge.mutate({ id: alert.alert_id, status: "seen" })}>Đã xem</Button><Button className="min-h-9 px-3" isLoading={acknowledge.isPending} onClick={() => acknowledge.mutate({ id: alert.alert_id, status: "safe" })}><CheckCircle2 size={15} /> Tôi an toàn</Button><Button variant="danger" className="min-h-9 px-3" isLoading={acknowledge.isPending} onClick={() => acknowledge.mutate({ id: alert.alert_id, status: "need_help" })}><HandHelping size={15} /> Cần hỗ trợ</Button>{alert.audio_available && <Button variant="ghost" className="min-h-9 px-3" onClick={() => void playAudio(alert.alert_id)}><Volume2 size={15} /> Nghe</Button>}</div>{alert.acknowledged_at && <p className="mt-3 text-xs text-muted">Trạng thái: {alert.acknowledgement_status}</p>}</article>)}</div>
  </div>;
}
